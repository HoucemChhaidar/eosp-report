...
The first sprint focuses on establishing the fundamental authentication infrastructure and end user management capabilities for the Credix mobile payment application. This sprint implements a three-tier role-based access control system distinguishing between Super Admin, Admin, and End User, with automatic wallet creation upon end user account setup. The following table presents the user stories prioritized for this sprint:
...
\subsubsection{Manage End Users}
...
\begin{longtable}{|p{4cm}|p{11cm}|}
  ...
  \textbf{Description} & This use case allows the Admin to:
    ...
    \textbf{Postconditions} & End user accounts are created, updated, or deleted; wallets are automatically created for new end users. \\ \hline
    \textbf{Nominal Scenario} &
      \begin{enumerate}[nosep,leftmargin=*]
        \item The Admin accesses the end user management interface.
        ...
        \item For a new end user, the system automatically creates a wallet.
        \item The system confirms the update of end user accounts.
      \end{enumerate} \\ \hline
    \textbf{Alternative Scenarios} &
      \begin{enumerate}[nosep,leftmargin=*]
        \item If a required field is empty, the system displays « Required Field » and blocks the action.
        \item If the email already exists, the system displays « End User already exists ».
...
\textbf{Postconditions} & The wallet is created, updated, deleted, or viewed, and its status is updated in the system. \\ \hline
  \textbf{Nominal Scenario} &
    \begin{enumerate}[nosep,leftmargin=*]
      ...
      \item The Admin selects an end user and chooses « Create », « Update », « Delete », or « View ».
  ...
  \textbf{Alternative Scenarios} &
    \begin{enumerate}[nosep,leftmargin=*]
      \item If the end user already has a wallet, the system displays « Wallet already exists ».
...
\textbf{Postconditions} & The user is authenticated, receives a JWT token, and accesses the role-appropriate interface. \\ \hline
  \textbf{Nominal Scenario} &
    \begin{enumerate}[nosep,leftmargin=*]
      ...
      \item The user is redirected to the appropriate interface (Super Admin dashboard, Admin dashboard, or End User wallet).
    \end{enumerate} \\ \hline
  \textbf{Alternative Scenarios} &
    \begin{enumerate}[nosep,leftmargin=*]
...
\subsubsection{End User Creation Sequence Diagram}

This diagram shows the end user creation process initiated by an Admin. The Admin submits end user details through the interface, which are processed by the user controller. The system validates the data, checks for existing end users, creates the new end user account, and automatically triggers wallet creation for the new end user.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/seq_user_creation.png}
\caption{End User Creation Sequence Diagram}
...
In this section, we present the main interfaces developed during Sprint 1, focused on secure authentication and end user management. The implementation covers both web application interfaces for Super Admins and Admins, and end user interfaces for End Users. Each interface is illustrated with screenshots and accompanied by functionality descriptions.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{images/web_sign_in}
  \caption{Web Authentication Interface}
  \label{fig:sprint1_web_auth}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{images/mobile_sign_in}
  \caption{Mobile Authentication Interface}
  \label{fig:sprint1_mobile_auth}
\end{figure}

— \textbf{Key Features:}
\begin{itemize}[nosep,leftmargin=*,label=•]
\item Email and password input fields with validation
\item Role-based redirection (Super Admin dashboard, Admin dashboard, or End User wallet)
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/web_admins_list}
  \caption{Super Admin Dashboard - Admin Management Interface}
...
\subsubsection{End User Management Interface}

The end user management interface allows Admins to create, update, delete, and view end user accounts.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/web_users_list}
  \caption{End User Management Interface - Admin View}
  \label{fig:sprint1_user_management}
\end{figure}

— \textbf{Key Features:}
\begin{itemize}[nosep,leftmargin=*,label=•]
\item End user management: Create, update, delete, and view end user accounts
\item Bulk operations for efficient end user onboarding
\item End user status monitoring and account activation controls
...
\item System automatically generates wallets for new end users
\item Admin can monitor end user activity and wallet status
\end{itemize}

\subsubsection{Wallet Management Interface}

The wallet management interface enables Admins to manage end user wallets with balance and status controls.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/web_wallets_list}
  \caption{Wallet Management Interface - Admin View}
  \label{fig:sprint1_wallet_management}
\end{figure}

— \textbf{Key Features:}
\begin{itemize}[nosep,leftmargin=*,label=•]
\item Wallet management: Create, update, delete, and view wallets with balances and status
\item Real-time balance monitoring and updates
\item Wallet activation and deactivation controls
\item Transaction history access per wallet
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{End User Mobile Interface - Wallet Home Screen}
...
\item Clear navigation and end user-friendly interface design
