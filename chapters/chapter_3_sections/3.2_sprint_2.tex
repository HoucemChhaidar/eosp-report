...
\subsubsection{Generate Time-bound Barcode}

\begin{longtable}{|p{3cm}|p{11cm}|}
\caption{Description of Use Case « Generate Time-bound Barcode »}
\label{tab:uc_generate_timebound_barcode} \\
\hline
...
\item End user can regenerate barcode as needed
\end{itemize} \\
\hline
\textbf{Main Flow} & 
\begin{enumerate}[nosep,leftmargin=*]
\item End user opens wallet application and navigates to payment section
\item End user selects "Generate Payment Barcode" option
\item System retrieves end user's wallet tokenizedId from database
...
\item End user can regenerate new barcode if needed
\end{enumerate} \\
\hline
\textbf{Alternative Flow} & 
\begin{itemize}[nosep,leftmargin=*]
\item \textbf{A1:} Barcode expires - End user can generate new barcode immediately
\item \textbf{A2:} Multiple payment attempts - End user can generate multiple barcodes in sequence
\end{itemize} \\
\hline
...
\subsubsection{View Real-time Transaction Confirmation}

\begin{longtable}{|p{3cm}|p{11cm}|}
\caption{Description of Use Case « View Real-time Transaction Confirmation »}
\label{tab:uc_view_realtime_transaction_confirmation} \\
\hline
...
\textbf{Preconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item End user has initiated a payment transaction
\item Payment processing is in progress
\end{itemize} \\
\hline
\textbf{Postconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item End user receives immediate transaction confirmation
\item Wallet balance is updated in real-time
\item Transaction is recorded in end user's history
\end{itemize} \\
\hline
\textbf{Main Flow} & 
\begin{enumerate}[nosep,leftmargin=*]
\item End user initiates a payment transaction
...
\item End user receives real-time notification of successful payment
\item System displays transaction confirmation with details (amount, timestamp, store, balance)
\item End user can view digital receipt immediately
\end{enumerate} \\
\hline
\textbf{Alternative Flow} & 
\begin{itemize}[nosep,leftmargin=*]
...
\subsubsection{View Balance}

\begin{longtable}{|p{3cm}|p{11cm}|}
\caption{Description of Use Case « View Balance »}
\label{tab:uc_view_balance} \\
\hline
...
\textbf{Preconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item End user is authenticated and has access to wallet
\item Wallet exists and is active in the system
\end{itemize} \\
\hline
\textbf{Postconditions} & 
...
\item End user has visibility into available funds
\end{itemize} \\
\hline
\textbf{Main Flow} & 
\begin{enumerate}[nosep,leftmargin=*]
\item End user opens wallet application home screen
\item System automatically retrieves current wallet balance
\item System displays balance prominently on home interface
\item End user can refresh to update balance manually
...
\subsubsection{View Transactions History}

\begin{longtable}{|p{3cm}|p{11cm}|}
\caption{Description of Use Case « View Transactions History »}
\label{tab:uc_view_transactions_history} \\
\hline
...
\textbf{Preconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item End user is authenticated and has wallet access
\item End user has performed at least one transaction (or none for empty state)
\end{itemize} \\
\hline
\textbf{Postconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item Complete transaction history is displayed chronologically
\item End user can access detailed information for each transaction
\item Transaction data is properly formatted and searchable
\end{itemize} \\
\hline
\textbf{Main Flow} & 
\begin{enumerate}[nosep,leftmargin=*]
\item End user navigates to "Transaction History" from main menu
\item System retrieves all transactions associated with end user's wallet
...
\item End user can tap on any transaction to view detailed information
\item Detailed view includes: transaction ID, timestamp, description, receipt
\item End user can filter transactions by date range, amount, or store
\item End user can search transactions using keywords
\end{enumerate} \\
\hline
\textbf{Alternative Flow} & 
\begin{itemize}[nosep,leftmargin=*]
...
\item \textbf{A3:} Export functionality - End user can export transaction history as PDF/CSV
\item \textbf{A4:} Print receipt - End user can print individual transaction receipts
\end{itemize} \\
\hline
\textbf{Exception Flow} & 
\begin{itemize}[nosep,leftmargin=*]
...
\subsubsection{View Stores}

\begin{longtable}{|p{3cm}|p{11cm}|}
\caption{Description of Use Case « View Stores »}
\label{tab:uc_view_stores} \\
\hline
...
\textbf{Postconditions} & 
\begin{itemize}[nosep,leftmargin=*]
\item List of stores is displayed to the end user
\item End user can view store details including name, type, and location
\item End user can navigate to store locations
\end{itemize} \\
\hline
\textbf{Main Flow} & 
\begin{enumerate}[nosep,leftmargin=*]
\item End user navigates to "Stores" from main menu
\item System retrieves list of available stores from database
\item System displays stores with name, description, and type
\item End user can view store details including location (latitude, longitude)
\item End user can filter stores by type (Retail, Restaurant, Cafe, Clothing, Grocery, Pharmacy, Electronics, Bookstore, Gas Station, Bakery)
\item End user can search for specific stores by name
\item End user can view store location on map
\end{enumerate} \\
\hline
\textbf{Alternative Flow} & 
\begin{itemize}[nosep,leftmargin=*]
...
\item \textbf{A3:} Favorite stores - End user can mark stores as favorites for quick access
\end{itemize} \\
\hline
\textbf{Exception Flow} & 
\begin{itemize}[nosep,leftmargin=*]
